...
Feature: CoinBase
  ...
  Scenario: Get Currency codes and names
    ...
    Given I want to call API Endpoint "{{coinBaseURL}}"
    ...
    And Response body JSON matches schema "data/response_schemas/getCoinBase_200.json"
    ...
    And I store JSON Response body node "data.get(id=ARS).name" as "ArgentineCurrencyName" in the scenario scope
  ...
  Scenario: Get Currency codes and names
    ...
    Given I want to call API Endpoint "{{coinBaseURL}}"
    And   I provide headers as csv file "data/headers/coinBase.csv"
    And I provide query parameter "id" as "HKD"
    And  I send "GET" request
    Then Response has Status code: "200"
    And Response has ALL the headers from data Table:
      |Content-Type           |
